  ...
  */		
  _uploadXhr: function() {
    var self = this,
      ...
      if (false === settings.startXHR.call(self, filename, fileSize, settings)) {
      //if (false === settings.startXHR(filename, fileSize)) {
        self.enable();
        return;
      }			
...
Util.Uploader = function(options){

    this._setting = {
        button: null,
        url: BASE_DIR+'file/upload',
        progressUrl:BASE_DIR+'file/progress',
        name:'upfile',
        ...
        startXHR: function(filename, size, settings) {
            var self = this;

            if (size >= 2000) {
                //errBox.innerHTML = 'File size exceeds 500K limit';
        ...
        onSubmit: function(file, ext) {
            ...
            if (ext !== 'png' && ext !== 'jpg' && ext !== 'jpeg' && ext !== 'gif') {
                //errBox.innerHTML = 'Invalid file type. Please select a PNG, JPG, or GIF image.';
                return false;
            }
            //errBox.innerHTML = '';
            //btn.value = 'Uploading...';
        ...
        onComplete:	function(file, response) {
            if (response.success === true) {
                //picBox.innerHTML = '<img src="file/view/' + encodeURIComponent(response.file) + '">';
            } else {
    ...
    Util.extendObj(this._setting, options);

}
Util.Uploader.prototype = {
    bind:function(){
        var self = this;
        var btnName = self._setting.button;

        self._setting.button = document.getElementById(btnName),
        self.pic_uploader = new ss.SimpleUpload(self._setting);
    },
    onExeed:function(){
        alert('文件过大');
    }
